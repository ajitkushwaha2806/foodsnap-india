    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      {Array.from({ length: limit }).map((_, i) => (
        <div key={i} className="cursor-pointer animate-pulse">
          <div className="overflow-hidden border rounded-md p-3 border-gray-200 dark:border-gray-800 bg-white dark:bg-zinc-900 shadow-sm">
            <div className="relative rounded-md overflow-hidden shadow-sm">
              <div className="w-full h-48 bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                <ImageIcon className="text-gray-300 dark:text-gray-700 w-8 h-8" />
              </div>
            </div>
            <div className="mt-3 h-4 bg-gray-200 dark:bg-gray-700 rounded-md w-3/4" />
          </div>
        </div>
      ))}
    </div>
